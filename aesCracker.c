...
void testDecrypt(uint8_t* key, uint8_t* ciphertext, string *dictionary){
	printf("Test Case: \n");
...
int decrypt(uint8_t* key, uint8_t* ciphertext, string *dictionary){
	...
	for(i = 0; i<(SIZE-1); i+=16){
  	...
  	//Return false if out of range ASCII value
  	if (max > 122){
  		return false;
  	}
	...
	for(i = 0; i < DICTIONARY_LEN; i++){
  	...
  	char *outputKey = (char *)(intptr_t)key;
  	printf("%s\n", outputKey);
  	return true;
}
...
// {
// 	key_options[] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,
...
// 	for (int k = 0; k<sizeof(key_options); k++) 
// 	{
// 		//call the decryption function each time
// 		//void testDecrypt(string *dictionary)
// 		testDecrypt_1(key_options[k]);
// 	}
// }
