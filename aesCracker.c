...
int decrypt(uint8_t* key, uint8_t* ciphertext, int size){
	int i, count;
	uint8_t buffer[SIZE];
...
int main(){
	...
	start = clock();
	while(!check){
		check = pthread_create(&threads[0], NULL, decryptParallel, (void *)&threadDataArray[0]);
		incrementKey(threadDataArray[0].key);
	}
	end = clock();
